...
(
    ...
    ./tests/quick.sh --very-fast --logs --version "$OLD_VERSION" --volume "$VOLUME" "$GAMESERVER" > "$log_downgrade" || true 
    ...
    elif ! ./tests/quick.sh --very-fast --logs --version "$VERSION" --volume "$VOLUME" "$GAMESERVER" > "$log_update"; then
